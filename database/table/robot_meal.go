...
func (m *Meal) TableName() string {
	return "robot_meal"
}

func GetMealByNameAndFlag(name string, flag int) (data Meal) {
	database.DB().Where("name = ? and flag = ?", name, flag).Find(&data)
	return
}

func GetRollMeal(flag int) string {
	var s []string
	database.DB().Table("robot_meal").Select("name").Where("flag = ?", flag).Scan(&s)
	if len(s) == 0 {
	...
	return utils.MicsSlice(s, 1)[0]
}

func CreateMeal(data Meal) (err error) {
